...
CONTRACT_PATH = Path(__file__).parent.parent / "contracts" / "civora.py"
...
def deploy(direct_deploy, sender=None):
    ...
    def windows_safe_unlink(path):
        try:
            ...
            if getattr(error, "winerror", None) != 32 or Path(path).parent != Path(os.getenv("TEMP", "")):
                raise

    os.unlink = windows_safe_unlink
    try:
        return direct_deploy(str(CONTRACT_PATH), sdk_version="v0.6.0-rc5")
    finally:
        os.unlink = unlink


def address_hex(raw: bytes) -> str:
    import genlayer as gl
    return gl.Address(raw).as_hex
...
def mock_llm_comparability(vm, comparability: str = "COMPARABLE", reason: str = "Official revision"):
    ...
    # The v0.3 direct-mode adapter decodes one JSON transport layer before
    # handing text to exec_prompt(response_format="json"). Preserve the text
    # payload expected by the SDK by encoding the mocked response envelope.
    vm.mock_llm(r"(?s).*comparability.*", json.dumps(json.dumps({
        "catalog": {
...
def test_consumer_namespace_binding(direct_vm, direct_deploy, direct_alice, direct_bob):
    ...
    assert binding_key == f"{address_hex(direct_bob)}:cpi-policy-dept"

    resolved_id = contract.get_consumer_binding(address_hex(direct_bob), "cpi-policy-dept")
...
def test_pagination_and_views(direct_deploy, direct_alice):
    ...
    found_id = contract.get_owner_nonce_trigger(address_hex(direct_alice), "nonce-p2")
...
def test_upgrade_contract_auth(direct_vm, direct_deploy, direct_alice, direct_bob):
    contract = deploy(direct_deploy, direct_alice)
    assert contract.get_upgrader().as_bytes == direct_alice
