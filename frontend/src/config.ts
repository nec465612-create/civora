...
export function validateConfig(): AppConfig {
  const rpcUrl = (import.meta.env.VITE_GENLAYER_RPC_URL || 'https://studio-dev.genlayer.com/api').trim();
  const chainIdStr = (import.meta.env.VITE_GENLAYER_CHAIN_ID || '61997').toString().trim();
  const contractAddress = (import.meta.env.VITE_CONTRACT_ADDRESS || '').trim();
  const explorerBaseUrl = 'https://explorer-studio-dev.genlayer.com';
  ...
  if (!contractAddress) {
    return {
      ...
      contractAddress: '',
      isConfigured: false,
      configError: 'The Civora contract is not deployed on Studio Next yet.',
      explorerBaseUrl,
    };
  }
