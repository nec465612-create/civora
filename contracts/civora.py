...
BLS_REQUEST_HEADERS = {
    "User-Agent": "Civora/1.0 (Public Research)"
}
...
def _parse_prompt_json(value) -> dict:
...
# Contract Class
# ---------------------------------------------------------------------------
class Civora(gl.contract.Contract):
    trigger_count: gl.u32
    trigger_ids: gl.storage.DynArray[str]
    triggers: gl.storage.TreeMap[str, str]
    vintage_counts: gl.storage.TreeMap[str, gl.u32]
    vintages: gl.storage.TreeMap[str, str]
    consumer_bindings: gl.storage.TreeMap[str, str]
    owner_nonces: gl.storage.TreeMap[str, str]
    active_canonical_keys: gl.storage.TreeMap[str, str]
    upgrader: gl.Address

    def __init__(self):
        self.trigger_count = gl.u32(0)
    ...
    def create_trigger(
        ...
        self.vintage_counts[trigger_id] = gl.u32(0)
        ...
        self.trigger_count = gl.u32(int(self.trigger_count) + 1)
        return trigger_id
    ...
    def freeze_trigger(self, trigger_id: str) -> str:
        ...
        _require(
            _sender_hex().lower() == str(trigger.get("owner", "")).lower(),
...
"""
                            metadata_result = _parse_prompt_json(
                                gl.nondet.exec_prompt(metadata_prompt, response_format="json")
                            )
...
"""
        ...
        return gl.vm.run_nondet(evaluate, validate)
    ...
    def observe_initial(self, trigger_id: str) -> str:
        ...
        self.vintage_counts[trigger_id] = gl.u32(1)
    ...
    def revalidate_trigger(self, trigger_id: str) -> str:
        ...
        v_total = int(self.vintage_counts.get(trigger_id, gl.u32(0)))
        ...
        if is_new_vintage:
            ...
            self.vintage_counts[trigger_id] = gl.u32(v_total + 1)
    ...
    # -----------------------------------------------------------------------
    @gl.public.view
    def get_trigger_count(self) -> gl.u32:
    ...
    @gl.public.view
    def get_triggers_page(self, offset: gl.u32, limit: gl.u32) -> str:
    ...
    @gl.public.view
    def get_vintage_count(self, trigger_id: str) -> gl.u32:
        _require(self.triggers.get(trigger_id, "") != "", "Trigger not found")
        return self.vintage_counts.get(trigger_id, gl.u32(0))

    @gl.public.view
    def get_vintage(self, trigger_id: str, index: gl.u32) -> str:
        idx = int(index)
        v_total = int(self.vintage_counts.get(trigger_id, gl.u32(0)))
    ...
    @gl.public.view
    def get_vintages_page(self, trigger_id: str, offset: gl.u32, limit: gl.u32) -> str:
        _require(self.triggers.get(trigger_id, "") != "", "Trigger not found")
        v_total = int(self.vintage_counts.get(trigger_id, gl.u32(0)))
        off = int(offset)
        lim = min(int(limit), PAGE_SIZE)
